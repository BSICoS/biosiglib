...
After the release is created, the workflow dispatches propagation to Biosigmat and Biosigpy automatically. Repository secrets `BIOSIGLIB_PROPAGATION_TOKEN` and `BIOSIGPY_PROPAGATION_TOKEN` must be configured in `BSICoS/biosiglib`. Each token should be scoped to its corresponding target repository (`BSICoS/biosigmat` or `BSICoS/biosigpy`); for fine-grained personal access tokens, GitHub requires `Contents: write` permission to create `repository_dispatch` events. The expected downstream result is one pull request in each implementation repository updating `conformance.json` to the new Biosiglib release.
